Shader "Custom/Disney_Principled_PBR"
{
    Properties
    {
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                //#ifdef _AdditionalLights
	                ...
	                {
                        Light additional_light = GetAdditionalLight(index, shadowCoord);
                        half3 additional_lightDir = normalize(additional_light.direction);
                        half3 additional_halfVector = normalize(additional_lightDir+viewDir);
                        float additional_nh = max(saturate(dot(i.worldNormal, additional_halfVector)), 0.000001);
                        float additional_nl = max(saturate(dot(i.worldNormal, additional_lightDir)), 0.000001);
                        float additional_lh = max(saturate(dot(additional_lightDir, additional_halfVector)), 0.000001);
                        half additional_D = GGXTerm(additional_nh,sqRoughness);
                        half additional_G = SmithJointGGXVisibilityTerm(additional_nl,nv,sqRoughness);
                        half3 additional_F = FresnelTerm(F0,additional_lh);
                        atten = additional_light.distanceAttenuation;
                        half3 additional_diff = 0;
                        half3 additional_spec = 0;
                        additional_diff = kd * DisneyDiffuse(nv,additional_nl,additional_lh,roughness)*additional_nl*additional_light.color;
                        additional_spec = additional_D*additional_G*additional_F*additional_light.color*additional_nl*PI;

                        lightResult += (additional_diff + additional_spec)*atten;
                    }
                //#endif
